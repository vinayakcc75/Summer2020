...
class BookSlot extends React.Component {
  ...
  onChange = async (date) => {
    ...
    await this.setState({ current_date: date }, () =>
      ...
      .then(async (ret) => {
        if (ret.status === true) {
          if (ret.message !== undefined) {
            ...
            this.setState({ disabledSlots: temp }, () =>
  ...
  componentDidMount = () => {
    fetch("http://localhost:8080/register/department", {
      ...
      .then(async (ret) => {
        if (ret.status === true) {
          await this.setState({ arr: ret.message }, () =>
    ...
    fetch("http://localhost:8080/department/doctor", {
      ...
      .then(async (ret) => {
        if (ret.status === true) {
          ...
          doctorsArray = [];
          await this.state.docArr.map((arr) => {
  ...
  getDocID = async (a) => {
    await this.setState({ doctor: a.value }, () =>
      console.log(this.state.doctor)
    );
    this.state.docArr.map((arr) => {
      if (arr.doctor === a.value) {
        this.setState({ doctor_id: arr.doctor_id }, () =>
